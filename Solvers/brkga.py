...
class BRKGA(BaseSolver):
    ...
    def __init__(self, graph: nx.Graph, num_k: int, num_d: int, num_subset_for_selection: int, pe: float, pm: float,
    ...
    def create_initial_population(self):
        return np.random.randint(self.g.number_of_nodes(), size=self.population_size)
    ...
    def top_k_random_key_sel(self, random_keys):
        return set(np.argsort(random_keys)[-self.k:])

    def solve(self):
        population = self.create_initial_population()
        e = int(self.pe * self.population_size)
        m = int(self.pm * self.population_size)
